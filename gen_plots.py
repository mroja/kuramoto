...
def plot_step(params):
    ...
    with open(f_name, 'r') as f:
        ...
        step_ = int(step_); assert(step_ == step)
        N = int(N)
        r = float(r)
        mean = float(mean)
...
def gen_video(dump_dir, subdir_name, framerate):
    ...
    cmd = 'ffmpeg -y -start_number 1 -framerate '+str(framerate)+' -i ' + pattern + ' -q:v 1 -vcodec mpeg4 ' + out_video
    #print('Executing: ' + cmd)
    os.system(cmd)
...
if __name__ == '__main__':
    ...
    def filter_files(seq):
        for el in seq:
            if 'r' not in el and 'mean' not in el:
                yield el
